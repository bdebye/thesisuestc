...
|\discipline{#1} | 专业研究领域中文名 | 无 |

> [!note]
>*注：根据[电子科技大学研究生学位论文撰写规范（2022年1月修订）](https://gr.uestc.edu.cn/xiazai/114/3917)相关要求，专业学位研究生（专业硕士生与工程博士生）应当分开填写其对应的专业学位类别以及专业学位领域。
> 使用`\major{#1}{#2}` 填写你的专业学位类别（比如`电子信息`）
...
使用BibTeX录入参考文献由`\thesisbibliography`命令导入`*.bib`文件数据库，参考文献风格自动设置为`thesis-uestc`。模板会自动统计参考文献数目，当数目超过100时自动调整编号的宽度，无需手动指定选项。
