...
频域方法的研究开展较早，目前应用广泛的包括：矩量法（MOM）\cite{bibson,martin}及其快速算
法多层快速多极子（MLFMA）\cite{wangchangqing}方法、有限元（FEM）\cite{hujun,jianmingjin}方法、自适应积分（AIM）
\cite{jianmingjin}方法等，这些方法是目前计算电磁学商用软件
...
\section{时域积分方程时间步进算法阻抗矩阵的存储}
时域阻抗元素的存储技术也是时间步进算法并行化的关键技术之一\cite{bibson}，采用
